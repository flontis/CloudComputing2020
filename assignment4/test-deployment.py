...
class DeploymentTester(object):
    ...
    def extract_header(self, response, header_name):
        if header_name not in response.headers:
            raise Exception("Missing header %s in response. Received headers: %s" % (
                header_name, list(response.headers.keys())))
    ...
    def check_set_len(self, the_set, set_name, expected_len):
        for l in expected_len:
            if len(the_set) == l:
                return 0
        print("Test failure: Unexpected number of encountered %s: Expected %s, have %s" % (
            set_name, expected_len, len(the_set)))
        return 1
    ...
    def validate(self):
        ...
        result = 0
        result += self.check_set_len(self.backend_containers,
                                     "backend containers", [NUM_BACKEND_CONTAINERS])
        result += self.check_set_len(self.frontend_containers,
                                     "frontend containers", [NUM_FRONTEND_CONTAINERS])
        expected_replies = NUM_FRONTEND_CONTAINERS * NUM_BACKEND_CONTAINERS
        result += self.check_set_len(self.replies, "replies", [
                                     expected_replies, int(expected_replies/2), int(expected_replies*3/4)])
        if result == 0:
